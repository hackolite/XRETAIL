...
def analyze(zoomed_im):
    ...
    EAN_STR = pytesseract.image_to_string(im2_arr, config=custom_config)

    EAN_STR = re.sub("[^0-9]", "", EAN_STR)
    print(EAN_STR)

    try:
        PRICE_STR = get_price(image_pillow=zoomed_im, model=model_ean)

    except:
        traceback.print_exc()
    return {"ean": EAN_STR, "price": PRICE_STR}
